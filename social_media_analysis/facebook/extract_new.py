...
def find_basic_info(url):
        ...
        req = requests.get(edu_url, cookies=cj)
        w=req.text
        category_containers=get_series_of_same_elememts('div',['class="tu1s4ah4"'],w,4)
        for category_container in category_containers:
            category_container_small=get_element('ul',['class="aahdfvyu bi6gxh9e"'],category_container)    
            # print ("category")
            # print(category_container)
            key_value
            key_value_containers=get_paren('li','ul',category_container_small)    
